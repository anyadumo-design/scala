...
function scala_options_schema(): array {
	return array(
		...
		'form' => array(
			...
			'fields' => array(
				...
				array(
					'key'   => 'telegram_token',
					'type'  => 'text',
					'label' => __( 'Telegram: токен бота', 'scala' ),
					'help'  => __( 'Видає @BotFather після команди /newbot — рядок з двокрапкою всередині. Порожньо — заявки в Telegram не йдуть.', 'scala' ),
				),
				array(
					'key'   => 'telegram_chat',
					'type'  => 'text',
					'label' => __( 'Telegram: канал або чат для заявок', 'scala' ),
					'help'  => __( 'ID на кшталт -1001234567890 або @назва публічного каналу. Простіше — збережіть токен і натисніть «Знайти чат» під формою.', 'scala' ),
				),
			),
		),
		...
		/* -------------------------------------------------- Відео ---- */
		'video' => array(
			'label'  => __( 'Відео «під ключ»', 'scala' ),
			'fields' => array(
				array(
					'key'   => 'video_file',
					'type'  => 'video',
					'label' => __( 'Вертикальне відео', 'scala' ),
					'help'  => __( 'Формат MP4, вертикальне (9:16). Без відео блок на сайті не показується зовсім.', 'scala' ),
				),
				array(
					'key'   => 'video_poster',
					'type'  => 'image',
					'label' => __( 'Кадр-заставка', 'scala' ),
					'help'  => __( 'Показується, доки відео не завантажилось. Якщо не обрати — перший кадр.', 'scala' ),
				),
				array(
					'key'     => 'video_eyebrow',
					'type'    => 'text',
					'label'   => __( 'Надпис над заголовком', 'scala' ),
				...
				array(
					...
					'type'    => 'text',
					'label'   => __( 'Заголовок', 'scala' ),
					'default' => 'Штори під ключ: від заміру до монтажу',
				),
				array(
					'key'     => 'video_text',
					'type'    => 'textarea',
					'label'   => __( 'Текст біля відео', 'scala' ),
					'rows'    => 4,
					'default' => 'Дизайнер приїжджає зі зразками, знімає розміри й показує, як тканина поводиться у вашому освітленні. Далі — розкрій, пошиття, карниз і монтаж.',
				),
				array(
					'key'       => 'video_points',
					'type'      => 'repeater',
					'label'     => __( 'Короткі пункти під текстом', 'scala' ),
					'row_label' => __( 'Пункт', 'scala' ),
					'fields'    => array(
						array(
							'key'   => 'text',
